export const Bio = {
  name: "Bhaskar Vora",
  roles: [
    "Full Stack Developer",
    "Android Developer",
    "UI/UX Designer",
    "Machine Learning Engineer",
    "Data Analyst / Scientist"
  ],
  description:
    "I am a highly motivated and adaptable professional with a strong enthusiasm for embracing new challenges. Driven by a passion for continuous learning, I am committed to delivering exceptional results. My positive outlook and growth mindset enable me to contribute effectively and strive for excellence in all my endeavors.",
  github: "https://github.com/bhaskarvora",
  resume:
    "https://drive.google.com/file/d/1zQPDZ7VAbXpokC5NfnEK0HepB8SK3qWY/view?usp=sharing",
  linkedin: "https://www.linkedin.com/in/bhaskar-vora-byv/",
  X: "https://x.com/bhaskarvora",
  insta: "https://www.instagram.com/bhaskaryvora/",
  
};

export const skills = [
  {
    title: "Frontend",
    skills: [
      {
        name: "React Js",
        image:
          "data:image/svg+xml;base64,",
      },
      {
        name: "Redux",
        image:
          "https://d33wubrfki0l68.cloudfront.net/0834d0215db51e91525a25acf97433051f280f2f/c30f5/img/redux.svg",
      },
      {
        name: "Next Js",
        image:
          "data:image/png;base64,",
      },
      {
        name: "HTML",
        image: "https://www.w3.org/html/logo/badge/html5-badge-h-solo.png",
      },
      {
        name: "CSS",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d5/CSS3_logo_and_wordmark.svg/1452px-CSS3_logo_and_wordmark.svg.png",
      },
      {
        name: "JavaScript",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/6/6a/JavaScript-logo.png/800px-JavaScript-logo.png",
      },
      {
        name: "Bootstrap",
        image:
          "https://getbootstrap.com/docs/5.3/assets/brand/bootstrap-logo-shadow.png",
      },
      {
        name: "Material UI",
        image:
          "data:image/png;base64,",
      },
      {
        name: "Flutter",
        image:
          "https://cdn-images-1.medium.com/max/1200/1*5-aoK8IBmXve5whBQM90GA.png",
      },
    ],
  },
  {
    title: "Backend",
    skills: [
      {
        name: "Node Js",
        image: "https://nodejs.org/static/images/logo.svg",
      },
      {
        name: "Express Js",
        image:
          "data:image/png;base64,",
      },

      {
        name: "Spring Boot",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSgu8cRVSRm96BF4FEJ4DA9IROZYI1wYR0RRA&s",
      },
      {
        name: "Graph Ql",
        image: "https://www.vectorlogo.zone/logos/graphql/graphql-icon.svg",
      },
      {
        name: "Python",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg",
      },
      {
        name: "MySQL",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg",
      },
      {
        name: "Postgresql",
        image: "https://www.postgresql.org/media/img/about/press/elephant.png",
      },
      {
        name: "MongoDB",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/mongodb/mongodb-original-wordmark.svg",
      },

      {
        name: "PHP",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTeOjNC4XG4smhQmXdkwxWS8bCXmce2pG151Q&s",
      },

      {
        name: "Django",
        image:
          "https://e7.pngegg.com/pngimages/159/366/png-clipart-django-python-computer-icons-logo-python-text-label.png",
      },
      {
        name: "Firebase",
        image: "https://www.vectorlogo.zone/logos/firebase/firebase-icon.svg",
      },
    ],
  },
  {
    title: "Android",
    skills: [
      {
        name: "Java",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/java/java-original.svg",
      },
      {
        name: "Kotlin",
        image:
          "https://www.vectorlogo.zone/logos/kotlinlang/kotlinlang-icon.svg",
      },
      {
        name: "XML",
        image:
          "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBMw6_RdwKQ9bDFfnKDX1iwMl4bVJEvd9PP53XuIw&s",
      },
      {
        name: "Android Studio",
        image:
          "https://developer.android.com/static/studio/images/new-studio-logo-1_1920.png",
      },
    ],
  },



  {
    title: "Data Science/Machine Learning",
    skills: [
      {
        name: "Python",
        image:
          "https://cdn3.iconfinder.com/data/icons/logos-and-brands-adobe/512/267_Python-512.png",
      },
      {
        name: "NumPy",
        image:
          "https://seeklogo.com/images/N/numpy-logo-479C24EC79-seeklogo.com.png",
      },
      {
        name: "Matplotlib",
        image:
          "https://image.pngaaa.com/242/4152242-middle.png",
      },
      {
        name: "Pandas",
        image:
          "https://seeklogo.com/images/P/pandas-logo-56829C6445-seeklogo.com.png",
      },

      {
        name: "Tensorflow",
        image:
          "https://logowik.com/content/uploads/images/tensorflow4903.jpg",
      },

      {
        name: "Scikit Learn",
        image:
          "https://icon2.cleanpng.com/20180817/btk/c3321f33f88b7d5dfff6c3e6feb8c41f.webp",
      },

      {
        name: "Tableau",
        image:
          "https://analyticstraininghub.com/wp-content/uploads/2020/10/icon-tableau.png",
      },

      {
        name: "PowerBi",
        image:
          "https://seeklogo.com/images/P/power-bi-microsoft-logo-E4FC8DE4A9-seeklogo.com.png",
      },

      {
        name: "NLP",
        image:
          "https://cdn-icons-png.flaticon.com/512/10129/10129318.png",
      },
      {
        name: "Neural Network",
        image:
          "https://www.iconpacks.net/icons/free-icons-6/free-artificial-intelligence-neural-network-icon-22174-thumb.png",
      },
    ],
  },
  {
    title: "Others",
    skills: [
      {
        name: "Git",
        image:
          "https://book.git-scm.com/images/logos/downloads/Git-Icon-1788C.png",
      },
      {
        name: "GitHub",
        image:
          "https://github.githubassets.com/images/modules/logos_page/GitHub-Mark.png",
      },
      {
        name: "Docker",
        image:
          "https://raw.githubusercontent.com/devicons/devicon/master/icons/docker/docker-original-wordmark.svg",
      },
      {
        name: "Netlify",
        image:
          "https://seeklogo.com/images/N/netlify-logo-BD8F8A77E2-seeklogo.com.png",
      },
      {
        name: "VS Code",
        image:
          "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9a/Visual_Studio_Code_1.35_icon.svg/512px-Visual_Studio_Code_1.35_icon.svg.png?20210804221519",
      },
      {
        name: "Postman",
        image:
          "https://www.postman.com/_ar-assets/images/postman-api-platform-social-preview-9420276277ad8d4e67e53ee548409999.jpg",
      },
  
      {
        name: "Figma",
        image:
          "https://s3-alpha.figma.com/hub/file/479066114/4e60317b-e1cd-44fd-adfb-9d225a561fd4-cover.png",
      },
    ],
  },
];

export const experiences = [
  {
    id: 0,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQHmb93ECz5mx29vbvLJniM22A1xccaXuE82w&s",
    role: "Data Scientist Intern",
    company: "Infolabz IT Services Ltd",
    date: "Dec 2022 - July 2023",
    desc: [
      "Developed and deployed machine learning models on ServiceNow data to analyze historical incident patterns, identify early warning signs, and predict potential service disruptions.",
      "Automated data quality monitoring in PowerBI using embedded Python.",
      "Optimized ETL jobs to improve the robustness of the pipeline and reduced the job timings by 40 percent.",
      "Developed Dashboards using Tableau Data Visualization tool."
    ],

    skills: [
      "Data Science,",
      "Python",
      "Machine Learning",
      "Generative AI",
      "Microsoft Power BI",
      "Tableau",
      "Docker",
      "AWS"
    ],
  
  },
  {
    id: 0,
    img: "https://d2q79iu7y748jz.cloudfront.net/s/_logo/cef3a5a00a5912fa6c61045273f783b6",
    role: "ReactJS Intern",
    company: "VNurture Technologies",
    date: "June 2022 - Nov 2022",
    desc: ["Developed an intuitive internal portal using React and Tailwindcss for creating extension specifications, streamlining the process for teams across the organization.",
    " Designed and integrated an easy-reordering feature on the customer account page, providing a seamless and efficient ordering experience for users." ,
   " Collaborated with a team in creating the foundation of the customer account UI extension using remote-ui and web worker technologies, laying the groundwork for future enhancements and customization options."],
    skills: [
      "ReactJS",
      "Redux",
      "NodeJs",
      "Material UI",
      "HTML",
      "CSS",
      "JavaScript",
      "TypeScript",
      "MongoDB",
      "Postman API",
      "TailWind CSS",
      "Stripe",
      "Multer",
      "FireBase",
    ],
  
  },
  {
    id: 1,
    img: "https://i.pinimg.com/280x280_RS/5a/0d/e1/5a0de1391c884cd4ee9ba5c4cbb3820e.jpg",
    role: "Python Django Intern",
    company: "CreArt Solutions",
    date: "Feb 2022 - June 2022",
    desc: ["Contributed effectively in authentication module and cart feature of an E-commerce website.",
      "Collaborated with team members using version control systems such as Git to organize modifications and assign tasks. " ,
    ],
    skills: [
      "Python",
      "Django",
      "Git",
      "Github",
      "JavaScript",
      "Docker",
      "PostgreSQL",
    ],
  },
  
  {
    id: 4,
    img: "https://i.pinimg.com/280x280_RS/5a/0d/e1/5a0de1391c884cd4ee9ba5c4cbb3820e.jpg",
    role: "Android Developer Intern",
    company: "CreArt Solutions",
    date: "June 2021 - Oct 2021",

    desc: [
      "During my tenure, I contributed to the development of the application's back-end cloud functions using Firebase and implemented a front-end with the MVVM structure" ,
      "This internship helped me enhance my skills in Android development and gave me an opportunity to work in a collaborative team environment."
    ],
    skills: [
      "Android",
      "Java",
      "Kotlin",
      "XML",
      "Node Js",
      "Cloud Firestore",
      "Firebase",
      "Figma",
    ],
   
  },
];

export const education = [
  {
    id: 0,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQU756fIyHLOd7YOpg4NPaOSaPv50yv-E4uZw&s",
    school: "Wilfrid Laurier University, Waterloo, Canada",
    date: "Sep 2023 - Dec 2024",
    grade: "3.7 GPA out of 4",
    desc: "I have taken courses in Practical Algorithm Design, Machine Learning, Android Programming, Advanced Parallel Programming, Data Analysis, Seminar in Tech Entrepreneurship, Cyber Attack and Defense, and Applied Cryptography. I am also an International Student Leader at Laurier International.",
    degree: " Masters of Applied Computing (Computer Science)",
  },
  {
    id: 1,
    img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRfktdqhQSWpJmqCfTzhHNjmvjgyFNbnLJ-w&s",
    school: "L.D. College of Engineering, Gujarat Technological University, Ahmedabad",
    date: "Oct 2020 - Jul 2023",
    grade: "9.23 CGPA out of 10",
    desc: "I completed my Bachelors in Computer Engineering in 2023 where I learnt subjects such as Data Structures, Digital Fundamentals, DBMS, Design Engineering, Probablity and Statistics, Operating System, Object Oriented Programming using Java, Computer Organization, Discrete Mathematics, Principles of Economics and Management, Analysis and Design of Algorithms, Professional Ethics, Computer Networks, Computer Graphics, Python for Data Science, Theory of Computation, Advanced Java Programming, Microprocessor and Interfacing, Web Programming, IOT and its Applications, Artificial Intelligence, Compiler Design, Cloud Computing, Machine Learning, Information Security, Digital Forensics, Internship ",
    degree: "Bachelors (B.E.) of Computer Engineering",
  },
  {
    id: 2,
    img: "https://images.shiksha.com/mediadata/images/1588064208phplgAE6e.jpeg",
    school: "Government Polytechnic Ahmedabad, Ahmedabad",
    date: "Aug 2017 - Aug 2020",
    grade: "9.96 CGPA out of 10",
    desc: "I completed my Diploma in Electronics and Communication Engineering in 2020 where I learnt Basics of Computer Engineering, Advanced Mathematics, Digital Logic Design, Programming in C, Microprocessor and Assembly Language Programming, Computer Networks, VLSI, Software Practices, Microcontroller, Digital Communication, Optical Communication, MicroController, Mobile Communication, Multimedia Communication, Entrepreneurship and Industrial Management, Electronic Circuit and Applications, Electornic Networks, Contributor Personality Development, Antenna and Wave Propagation, Industrial Electronics .",
    degree: "Diploma in Electronics and Communication Engineering, Gujarat Technological University, Ahmedabad",
  },
];

export const projects = [
  {
    id: 9,
    title: "ChatWave",
    date: "Jun 2024 - Aug 2024",
    description:
      "ChatWave is a modern chat application that enables real-time messaging between users. It offers features for user authentication, chat rooms, file sending, live notifications, private messaging, and administrative controls for managing users and messages. This application provides a seamless user experience with modern design and real-time updates.",
    image:
      "https://github.com/bhaskarvora/ChatWave_Frontend/blob/master/Screenshot%202024-08-30%20164128.png?raw=true",
    tags: [
      
      "ReactJS",
      "ExpressJS",
      "NodeJS",
      "Socket.IO",
      "React Js",
      "MongoDb",
      "RestApi",
      "Docker",
      "Redux",
    ],
    category: "web app",
    github: "https://github.com/bhaskarvora/ChatWave_Server",
    webapp: "",
  },


  {
    id:11,
    title: "Crafted Treasures",
    date: "Jun 2024 - Sep 2024",
    description:
      "CraftedTreasures is an innovative project designed to showcase and promote handcrafted treasures from artisans around the world. The project aims to create a vibrant online marketplace that connects skilled crafters with enthusiasts who appreciate unique, artisanal products.",
    image:
      "https://github.com/bhaskarvora/CraftedTreasures/blob/master/Home%20Page%20Crafted.png?raw=true",
    tags: [
      
      "Spring Boot",
      "Spring Security",
      "JWT",
      "MySQL",
      "JPA",
      "Razorpay API",
      "Angular",
      "Bootstrap",
      "Typescript",
      "Restful APIs",
    ],
    category: "web app",
    github: "https://github.com/bhaskarvora/CraftedTreasures",
    webapp: "",
  },
  {
    id: 0,
    title: "Ecommerce Website using MERN Stack",
    date: "Apr 2024 - Jun 2024",
    description:
      "Developed a full-stack web application that showcases a robust frontend application integrated with Firebase Authentication for secure user management, utilizing react-router-dom for dynamic routing and protected routes. The application features lazy-loaded pages and protected routes to ensure secure access based on user roles, enhancing both performance and security. The user interface is designed to be responsive and user-friendly, incorporating react-hot-toast for real-time feedback on user actions and API responses. State management is efficiently handled using Redux, with TypeScript providing type safety and improving developer experience by defining clear types for state and API interactions. Additionally, the project includes comprehensive error management, with detailed handling of API responses and navigation based on success or failure scenarios, ensuring a seamless and intuitive user experience.",
    image:
      "https://github.com/bhaskarvora/MERN-STACK-ECOMMERCE-WEBSITE-FRONTEND/blob/master/Screenshot%202024-09-01%20002658.png?raw=true",
    tags: ["React Js", "TypeScript", "MongoDB", "Firebase", "SCSS","React-Redux","Git","Javascript","Vite","Postman","Stripe"],
    category: "web app",
    github: "https://github.com/bhaskarvora/MERN-STACK-ECOMMERCE-WEBSITE-BACKEND",
    webapp: "",
    
  },
  {
    id: 1,
    title: "OYO Clone",
    date: "Mar 2024 - Apr 2024",
    description:
      "The OYO Clone project offers a range of features designed to replicate key aspects of an online booking platform. It includes a comprehensive user authentication system, allowing users to securely sign up and log in with their credentials. The project integrates Stripe for payment processing, enabling users to handle transactions seamlessly through a secure checkout flow. Additionally, the application connects to a MongoDB database to manage and store user data effectively. It employs robust error handling mechanisms to address issues related to both payment processing and API interactions, ensuring a smooth and reliable user experience. Overall, the OYO Clone combines essential functionalities for user management and payment processing in a cohesive and user-friendly web application.",
    image:
      "https://github.com/bhaskarvora/Oyo-Clone/blob/Remaining_Code/Screenshot%202024-08-29%20160019.png?raw=true",
    tags: [
      "Next.js",
      "MongoDb",
      "Tailwind CSS",
      "Axios",
      "React state hooks",
      "Stripe",
      "MongoDB",
      "JSON Web Tokens (JWT)",
    ],
    category: "web app",
    github: "https://github.com/bhaskarvora/Oyo-Clone",
    webapp: "",
  },
  {
    id: 2,
    title: "Automatic Essay Scoring using LSTM networks",
    date: "Mar 2024 - Apr 2024",
    description:
      "This  project delves into the domain of Automated Essay Scoring  (AES) with a primary focus on leveraging Long Short-Term  Memory (LSTM) networks for essay evaluation. Traditional grading methods often suffer from subjectivity and inefficiency,  whereas AES offers a consistent, unbiased, and expedited . Leveraging LSTM networks, renowned for their proficiency in processing sequential data and addressing vanishing gradient issues, holds promise for enhancing scoring accuracy and advancing educational insights.",

    image:
      "https://github.com/bhaskarvora/Automatic-Essay-Scoring-using-LSTM-networks/blob/main/327629999-7190e1fd-d034-4b6f-b651-51ef21af6cb7.png?raw=true",
    tags: ["Python", "LSTM Networks", "NLP", "Django", "Google News 300"],
    category: "machine learning",
    github: "https://github.com/bhaskarvora/Automatic-Essay-Scoring-using-LSTM-networks",
    webapp: "",
    
  },
 
  {
    id: 10,
    title: "BidRideGo",
    date: "Oct 2023 - Nov 2023",
    description:
      "Developed user-friendly android app enabling seamless ride requests and driver bids. Implemented real-time bidding methodology optimizing driver-rider matching for an optimal experience. Efficient map integration for seamless pick-up and drop-off location entry, with automatic recognition of the currentlocation for effortless booking. Improved the overall ride-hailing experience by reducing wait times, increasing driver earnings, and offering riders more pricing options.",
    image:
      "https://github.com/bhaskarvora/Web-development/blob/main/327626660-91ebc601-e97d-4997-8e88-242b08bc3636.png?raw=true",
    tags: ["Java", "Google Maps API", "Open Cage Data API","XML","Firebase"],
    category: "android app",
    github: "https://github.com/mitalipadiya/bidridego",
    webapp: "",
  },
  {
    id: 3,
    title: "Face Mask Detection using Convolutional Networks(CNN) ",
    date: "Oct 2022 - Nov 2022",
    description:
      "The main objective of this project is to design a system which captures user face and detects that mask is present or not. This project uses Convolutional Neural Network (CNN) for processing data and it uses MobileNetV2 data model which is light weight and used for image classification.",
    image:
      "https://user-images.githubusercontent.com/84896867/203959388-2d549271-9512-4859-88f9-966b7f89b494.png",
    tags: ["Python", "ML", "AI", "Image Processing","CNN","Tensorflow","OpenCV","Raspberry Pi"],
    category: "machine learning",
    github: "https://github.com/bhaskarvora/Face-Mask-Detection-using-Machine-Learning-and-Convolutional-neural-Networks",
    webapp: "",
  },

  {
    id: 8,
    title: "Movie Recommender System",
    date: "Oct 2022 - Nov 2022",
    description:
      "This project is a Movie Recommender System that suggests movies based on a selected movie's similarity to others. The system utilizes a machine learning model to determine the similarity between movies, allowing users to receive personalized movie recommendations. The application fetches movie details, including posters, from the TMDB (The Movie Database) API and displays them in a user-friendly interface using Streamlit.",
    image:
      "https://github.com/bhaskarvora/Movie-Recommender-System/blob/main/Screenshot%202024-09-01%20221222.png?raw=true",
    tags: ["Python", "Pandas", "Streamlit", "TMDB API", "Pickle", "Cosine Similarity Algorithm"],
    category: "machine learning",
    github: "https://github.com/bhaskarvora/Movie-Recommender-System",
    webapp: "",
  },
  {
    id: 4,
    title: "Krushi Bazaar (Agriculture Market)",
    date: "Aug 2022- Oct 2022",
    description:
      "A convenient method to buy/sell agro products such as crops, food grains, and vegetables online in wholesale or retail. Direct trade between buyers and farmers by eliminating the middleman between them. Renting of farming tools such as Hand Fork, Spade, Rake, Axe, Sprinklers, etc. Provides seeds, fertilizer, insecticides, pesticides, etc that help in crop production.",
    image:
      "https://user-images.githubusercontent.com/84896867/202774585-d2d1409b-3f5d-45b3-a072-1921b0ba3083.png",
    tags: ["HTML", "CSS", "JavaScript", "PHP","Bootstrap","JQuery","MySQL"],
    category: "web app",
    github: "https://github.com/bhaskarvora/Krushi-Bazaar",
    webapp: "",
  },
  {
    id: 5,
    title: "Women Be Safe",
    date: "Jun 2021 - Jun 2022",
    description:
      "Women Be Safe application can be used to help women in emergency situation. This app has panic button which transmitsthe GPS live location of an individual via SMS and rings the contact person.Here Geolocation API is used which returns a location and accuracy radius based on information about cell towers and WiFi nodes that the mobile client can detect.",
    image:
      "https://user-images.githubusercontent.com/84896867/202672807-01c14815-0dcf-4ffa-adec-f74a5c64b28e.png",
    tags: ["Java", "Android", "SQL", "Google Maps API"],
    category: "android app",
    github: "https://github.com/bhaskarvora/Women-Be-Safe",
    webapp: "",
  },
  {
    id: 6,
    title: "Event Management System",
    date: "Mar 2022 - Jun 2022",
    description:
      "An event management system assists event planners in planning, executing, and reporting events. In this application, an admin can add events like event name, event description, event price, event image. Users can enroll in events by providing their basic details such as name, email id, phone number. After successful enrollment users will get confirmed tickets. An admin can see users who have enrolled in which events and their details.",
    image:
      "https://user-images.githubusercontent.com/84896867/202760890-6fac6bac-4c84-41c9-be60-c39fbd8bf9cb.png",
    tags: ["Java", "Hibernate", "Servlet", "JSP","CSS","MySQL"],
    category: "web app",
    github: "https://github.com/bhaskarvora/event-management-system",
    webapp: "",
  },
  {
    id: 7,
    title: "Flutter Bird Game",
    date: "Dec 2020 - Jan 2021",
    description:
      "Flutter bird game using pygame for Computer Graphics Project",
    image:
      "https://github.com/bhaskarvora/flappy1/blob/main/Screenshot%202024-09-01%20213727.png?raw=true",
    tags: ["Python", "Pygame"],
    category: "",
    github: "https://github.com/bhaskarvora/flappy1",
    webapp: "",
  },
  {
    id: 8,
    title: "IOT based Automatic Working Robotic Arm",
    date: "Jan 2020- Apr 2020",
    description:
      "The main focus of our project is to design and develop a mechanism like a robotic arm for the lifting and movement of objects which is completely user friendly.robotic arm is designed with 4 Degrees Of Freedom (4-DOF) with 1-DOF for movement in X-axis and 3-DOF for movement in Y-axis. Here we have used a simple lightweight ABS material filament for the construction for physical assembly of our robotic arm. And 3D printing method is used to to fabricate all the parts of our robotic arm. For the accurate movement is X and Y axis, our robotic arm is equipped with 4 servo motors and a manual knobs consists of potentiometers for each servo to change the positions. And to control all the servos and to connect all the potentiometers here we have used a very popular AVR development board Arduino, which is open source computer hardware and easily available in market at affordable rates. And the Arduino IDE which we have used to program this board is also user friendly.",
    image:
      "https://user-images.githubusercontent.com/84896867/202794811-f3717577-2fea-4707-b52a-ca1297e6169d.png",
    tags: ["C++", "Arduino UNO", "Servo Motors", "Physical Assembly", "SMPS"],
    category: "",
    github: "https://github.com/bhaskarvora/IOT-based-Automatic-Working-Robotic-Arm",
    webapp: "",
  },
];

export const TimeLineData = [
  { year: 2017, text: "Started my Diploma Engineering journey learning C, C++" },
  { year: 2018, text: "Learnt Microprocessor, Digital Logic Design Concepts" },
  { year: 2019, text: "Founded JavaScript Mastery" },
  { year: 2020, text: "Shared my projects with the world" },
  { year: 2021, text: "Started my own platform" },
];
